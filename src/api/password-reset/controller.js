...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Coursewatch Backend account.<br>
        ...
        &mdash; Coursewatch Backend Team
      `
      return sendMail({ toEmail: email, subject: 'Coursewatch Backend - Password Reset', content })
    })
